(() => {
  ...
  function normalize(value) {
    ...
    const names = Array.isArray(source.names) ? source.names.map(String) : fallback.names;
    while (names.length < 5) names.push(`Person ${names.length + 1}`);
    return {
      participantCount: Math.min(5, Math.max(2, Number(source.participantCount) || 2)),
  ...
  function tokenFrom(value) {
    ...
    try {
      return new URL(raw).searchParams.get("group") || "";
    } catch {
      return raw.replace(/^.*group=/, "").split(/[&#]/)[0];
    }
  }
  ...
  function openGroup(groupData) {
    group = groupData;
    state = normalize(groupData.state);
    localStorage.setItem(GROUP_KEY, groupData.invite_token);
    localStorage.setItem(LOCAL_KEY, JSON.stringify(state));
    history.replaceState(null, "", `${location.pathname}?group=${groupData.invite_token}`);
    $("landing").classList.add("hidden");
    $("mainApp").classList.remove("hidden");
    $("currentGroup").textContent = groupData.name;
    const inviteLink = `${location.origin}${location.pathname}?group=${groupData.invite_token}`;
    $("inviteLink").textContent = inviteLink;
    $("qrImage").src = `https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=${encodeURIComponent(inviteLink)}`;
    subscribe();
    render(false);
  }
  ...
  function subscribe() {
    ...
    channel = db
      .channel(`fairwash-${group.group_id}`)
      .on("postgres_changes", {
        event: "UPDATE",
        schema: "public",
        ...
        state = normalize(payload.new.state);
        localStorage.setItem(LOCAL_KEY, JSON.stringify(state));
  ...
  function queueSave() {
    ...
    saveTimer = setTimeout(saveCloud, 300);
  }
  ...
  async function saveCloud() {
    ...
    saving = false;
    if (result.error) {
      $("syncStatus").textContent = "Offline – lokal gespeichert";
  ...
  function totals() {
    ...
    state.history.forEach(entry => {
      if (entry.person >= 0 && entry.person < scores.length) {
        scores[entry.person] += Number(entry.points) || 0;
  ...
  function nextPerson() {
    ...
    const candidates = scores.map((value, index) => value === minimum ? index : -1).filter(index => index >= 0);
    if (!state.history.length) return candidates[0];
    const last = state.history[state.history.length - 1].person;
  ...
  $("addBtn").onclick = () => {
    ...
    state.history.push({ person, amount, fat, difficulty, points: amount + fat + difficulty, createdAt: new Date().toISOString() });
    render();
